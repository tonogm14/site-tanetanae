...
// ── Slots que admiten multiples banners (sidebar) ─────────
const TT_MULTI_SLOTS = [ 'home-sidebar-top', 'home-sidebar-bottom', 'articulo-sidebar' ];
...
add_action( 'admin_enqueue_scripts', function ( $hook ) {
    ...
    wp_enqueue_media();
    wp_add_inline_script( 'jquery-core', tt_banners_js() );
} );
...
jQuery(function($) {
    ...
    $(document).on('click', '.tt-upload-btn', function(e) {
        ...
        mediaFrame.on('select', function() {
            ...
            $input.val(url);
            $prev.html('<img src="' + url + '" style="max-height:80px;margin-top:8px;border-radius:4px;border:1px solid #ddd;display:block">');
        });

        mediaFrame.open();
    });

    // Quitar imagen (dentro de un item)
    ...
    // Eliminar item completo (multi-slot)
    $(document).on('click', '.tt-delete-item', function(e) {
        e.preventDefault();
        if (confirm('Eliminar este banner?')) {
            $(this).closest('.tt-banner-item').remove();
        }
    });

    // Agregar nuevo banner (multi-slot)
    $(document).on('click', '.tt-add-banner', function(e) {
        e.preventDefault();
        var $section = $(this).closest('.tt-multi-slot');
        var slug     = $section.data('slug');
        var idx      = $section.find('.tt-banner-item').length;

        var html = '<div class="tt-banner-item" style="margin-bottom:12px">'
            + '<div class="tt-banner-block">'
            + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">'
            + '<span style="font-size:12px;font-weight:600;color:#555">Banner #' + (idx+1) + '</span>'
            + '<button type="button" class="button tt-delete-item" style="color:#b32d2e;border-color:#b32d2e">Eliminar</button>'
            + '</div>'
            + '<div class="tt-banner-row"><label>Imagen</label>'
            + '<div><input type="url" name="banner[' + slug + '][' + idx + '][image_url]" value="" placeholder="https://..." class="tt-image-url">'
            + '<div class="tt-preview"></div>'
            + '<button type="button" class="button tt-upload-btn">Subir / Seleccionar imagen</button></div></div>'
            + '<div class="tt-banner-row"><label>Enlace</label>'
            + '<input type="url" name="banner[' + slug + '][' + idx + '][link_url]" value="" placeholder="https://anunciante.com (opcional)"></div>'
            + '<div class="tt-banner-row"><label>Opciones</label>'
            + '<div class="tt-checks">'
            + '<label><input type="checkbox" name="banner[' + slug + '][' + idx + '][enabled]" value="1" checked> Mostrar</label>'
            + '<label><input type="checkbox" name="banner[' + slug + '][' + idx + '][new_tab]" value="1"> Abrir en nueva pestana</label>'
            + '</div></div>'
            + '</div></div>';
...
add_action( 'admin_menu', function () {
    add_menu_page( 'Banners', 'Banners', 'manage_options', 'tt-banners', 'tt_banners_page', 'dashicons-format-image', 60 );
} );

// ── Guardar ───────────────────────────────────────────────
function tt_banners_save() {
    if ( ! isset( $_POST['tt_banners_nonce'] ) ||
         ! wp_verify_nonce( $_POST['tt_banners_nonce'], 'tt_save_banners' ) ) return;

    foreach ( tt_banner_sections() as $slug => $label ) {
        $raw = $_POST['banner'][ $slug ] ?? [];

        if ( in_array( $slug, TT_MULTI_SLOTS ) ) {
            // Guardar array de banners
            $items = [];
            foreach ( $raw as $item ) {
                if ( ! is_array( $item ) ) continue;
                $img = esc_url_raw( $item['image_url'] ?? '' );
                if ( $img === '' ) continue; // omitir items sin imagen
                $items[] = [
                    'image_url' => $img,
                    'link_url'  => esc_url_raw( $item['link_url'] ?? '' ),
                    'enabled'   => isset( $item['enabled'] ) ? 1 : 0,
                    'new_tab'   => isset( $item['new_tab'] )  ? 1 : 0,
                ];
            }
            update_option( "tt_banner_{$slug}", $items );
        } else {
            // Banner unico (comportamiento original)
...
function tt_banners_page() {
    ...
    <div class="wrap">
        ...
        <?php if ( $saved ) : ?>
        <div class="notice notice-success is-dismissible"><p><strong>Banners guardados correctamente.</strong></p></div>
        <?php endif; ?>
        ...
        <style>
            ...
            .tt-banner-row input[type=url], .tt-banner-row input[type=text] { width:100%; padding:6px 9px; border:1px solid #ccc; border-radius:4px; font-size:13px; box-sizing:border-box; }
...
function tt_get_banners_rest() {
    ...
    foreach ( tt_banner_sections() as $slug => $label ) {
        if ( in_array( $slug, TT_MULTI_SLOTS ) ) {
            $items = get_option( "tt_banner_{$slug}", [] );
            if ( ! is_array( $items ) ) $items = [];
            $result[ $slug ] = array_values( array_map( function( $item ) {
                return [
